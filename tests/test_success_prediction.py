...
To use it : ./test_success_prediction.py > my_file.csv
It will write your data in the csv in this order :
Correct predictions count; Games count; Percent of the delta

Make sure to have an log/app.log in the root directory of the project
"""
import sys
import os
...
for index in range(9):
    ...
    for match in response:
        try:
            pred = Prediction(match["home_team_name"], match["away_team_name"], str(match["date"] + relativedelta(months=-6)), str(match["date"]))
